...
export default function ModelInsights() {
  ...
  const { predictedVsActual, residuals, shapValues, pdpData, correlationMatrix, features } = insights;

  // Calculate R² for the scatter plot
  const meanActual = predictedVsActual.reduce((sum, d) => sum + d.actual, 0) / predictedVsActual.length;
  const ssTotal = predictedVsActual.reduce((sum, d) => sum + Math.pow(d.actual - meanActual, 2), 0);
  const ssResidual = predictedVsActual.reduce((sum, d) => sum + Math.pow(d.residual, 2), 0);
            ...
            <CardContent>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                    ...
                    <XAxis
                      ...
                      dataKey="actual"
                      name="Actual WQI"
                      label={{ value: "Actual WQI", position: "insideBottom", offset: -10 }}
                    />
                    <YAxis
                      type="number"
                      dataKey="predicted"
                      name="Predicted WQI"
                      label={{ value: "Predicted WQI", angle: -90, position: "insideLeft" }}
                    />
                    <Tooltip
                          ...
                          return (
                            <div className="bg-white p-3 border border-gray-200 rounded shadow-lg">
                              ...
                              <p className="text-xs text-gray-600">
                                Error: {(payload[1].value - payload[0].value).toFixed(2)}
                              </p>
                            </div>
                          );
                        }
              ...
              <div className="mt-4 grid grid-cols-3 gap-4">
                <div className="p-3 bg-blue-50 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-blue-700">{r2Score}</p>
                </div>
                <div className="p-3 bg-green-50 rounded-lg">
                  <p className="text-xs text-green-600 font-medium">Mean Error</p>
                  <p className="text-2xl font-bold text-green-700">
                    {(residuals.reduce((sum, d) => sum + Math.abs(d.residual), 0) / residuals.length).toFixed(2)}
                  </p>
                </div>
                <div className="p-3 bg-purple-50 rounded-lg">
                  <p className="text-xs text-purple-600 font-medium">Std Dev</p>
                  <p className="text-2xl font-bold text-purple-700">
                    {Math.sqrt(
                      residuals.reduce((sum, d) => sum + Math.pow(d.residual, 2), 0) / residuals.length
                    ).toFixed(2)}
                  </p>
                </div>
              </div>
        ...
        >
          <Card className="mb-6">
            <CardHeader>
              ...
              <CardDescription>
                Visualize prediction errors to detect bias and variance patterns
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                    ...
                    <XAxis
                      ...
                      label={{ value: "Predicted WQI", position: "insideBottom", offset: -10 }}
                    />
                    <YAxis
                      type="number"
                      dataKey="residual"
                      name="Residual"
                      label={{ value: "Residual (Error)", angle: -90, position: "insideLeft" }}
                    />
                    <Tooltip
                      cursor={{ strokeDasharray: "3 3" }}
                      content={({ active, payload }) => {
              ...
              <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                ...
                <ul className="text-sm text-blue-700 space-y-1">
                  <li>• Random scatter around zero line = good model (no bias)</li>
                  <li>• Funnel shape = heteroscedasticity (variance increases)</li>
                  <li>• Curved pattern = model missing non-linear relationships</li>
                </ul>
              </div>
            </CardContent>
          </Card>
              ...
              <CardContent>
                    ...
                    >
                      ...
                      <Bar dataKey="importance" radius={[0, 8, 8, 0]}>
                        {shapValues.map((entry, index) => (
                          <Cell
                            key={`cell-${index}`}
                            fill={`hsl(${210 + index * 5}, 80%, ${60 - index * 3}%)`}
                          />
                        ))}
                      </Bar>
                    </BarChart>
                ...
                <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                  <p className="text-xs text-blue-800">
                    <strong>SHAP values</strong> explain individual predictions by showing
          ...
          >
            <Card className="h-full">
              ...
              <CardContent>
                        ...
                        content={({ active, payload }) => {
                          if (active && payload && payload.length) {
                            return (
                              <div className="bg-white p-3 border border-gray-200 rounded shadow-lg">
                                <p className="text-sm font-semibold">
                                  {selectedFeature}: {payload[0].payload.value.toFixed(2)}
                                </p>
                                <p className="text-sm">
                ...
                <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                  <p className="text-xs text-blue-800">
                    <strong>PDP</strong> shows how the prediction changes as a feature
                    varies, revealing non-linear relationships.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <div className="inline-block min-w-full">
                  <div className="grid gap-1" style={{ gridTemplateColumns: `40px repeat(${features.length}, 60px)` }}>
                    {/* Header row */}
                    <div></div>
                    {features.map((feature) => (
                      <div
                        ...
                        {features.map((feature2, j) => {
                          ...
                          return (
                            <div
                              ...
                              title={`${feature1} vs ${feature2}: ${correlation.toFixed(2)}`}
                            >
              ...
              <div className="mt-6 flex items-center justify-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-600">Weak Correlation</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-12 h-4 bg-linear-to-r from-blue-300 to-blue-500 rounded"></div>
                  <span className="text-xs text-gray-600">Moderate Correlation</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-12 h-4 bg-linear-to-r from-blue-600 to-blue-900 rounded"></div>
                  <span className="text-xs text-gray-600">Strong Correlation</span>
                </div>
              </div>
              ...
              <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                ...
                <ul className="text-sm text-blue-700 space-y-1">
                  <li>• Strong correlations (&gt; 0.7) indicate redundant features</li>
                  <li>• TDS and Conductivity typically show high correlation</li>
